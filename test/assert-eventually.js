  ...
  describe('On a promise fulfilled with the number 42', () => {
    ...
    describe('.eventually.equal(promise, 52)', () => {
      ...
      function shouldFailWithCorrectActual(promiseProducer) {
        it(
          'should return a promise rejected with an assertion error that has actual/expected properties ' +
            'correct',
          (done) => {
            expect(
      ...
      describe('expect', () => {
        shouldFailWithCorrectActual(() =>
          expect(promise).to.eventually.equal(52)
        );
      });
      describe('should', () => {
