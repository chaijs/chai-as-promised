  ...
  it('should return a fulfilled promise', (done) => {
    promiseProducer().then(
      ...
      (reason) =>
          ...
          new Error(
            `Expected promise to be fulfilled but it was rejected with ${reason.stack}`
          )
        )
    );
  });
  ...
  it('should return a promise rejected with an assertion error', (done) => {
    promiseProducer()
      .then(
        () => {
          throw new Error(
            'Expected promise to be rejected with an assertion error, but it was fulfilled'
          );
        },
        (reason) => {
          if (
            ...
            throw new Error(
              `Expected promise to be rejected with an AssertionError but it was rejected ` +
                `with ${reason}`
            );
          }
          ...
          if (
            ...
            !reason.message.includes(desiredMessageSubstring)
          ) {
            throw new Error(
              `Expected promise to be rejected with an AssertionError containing ` +
                `"${desiredMessageSubstring}" but it was rejected with ${reason}`
            );
          }

          if (
            nonDesiredMessageSubstring &&
            reason.message.includes(nonDesiredMessageSubstring)
          ) {
            throw new Error(
              `Expected promise to be rejected with an AssertionError not containing ` +
